...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    features = [float(f) for f in line.split(',')]
    return LabeledPoint(features[0], features[1:])

parsedSamplePoints = [(parsePoint(rec)) for rec in samplePoints]
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(lambda rec: parsePoint(rec))
...
parsedTrainData.map(lambda rec: parsePoint(rec))
parsedValData.map(lambda rec: parsePoint(rec))
parsedTestData.map(lambda rec: parsePoint(rec))
...
def calcRMSE(labelsAndPreds):
    ...
    return np.sqrt((labelsAndPreds
           .map(lambda rec: squaredError(rec[0], rec[1]))
           .reduce(lambda a, b: a + b)) / (1.0*labelsAndPreds.count()))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda rec: gradientSummand(w, rec)).reduce(lambda a, b: a + b)
...
labelsAndPreds = parsedValData.map(lambda rec: getLabeledPrediction(weightsLR0, rec))
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, np.hstack((lp.features, [x * y for x in lp.features for y in lp.features])))

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda rec: twoWayInteractions(rec))
valDataInteract = parsedValData.map(lambda rec: twoWayInteractions(rec))
testDataInteract = parsedTestData.map(lambda rec: twoWayInteractions(rec))
