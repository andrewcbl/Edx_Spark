...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRatings = sum(IDandRatingsTuple[1]) / (1.0 * numRatings)
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda m : getCountsAndAverages(m))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                             ...
                             .map(lambda (id, (name, (nr, ar))) : (ar, name, nr)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (ar, name, nr) : nr > 500)
...
trainingAvgRating = trainingRDD.map(lambda rec: rec[2]).mean()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda rec: (rec[0], rec[1], trainingAvgRating))
...
# movieNameWithIDAvgRatingsRDD = (moviesRDD
#                              .join(movieIDsWithAvgRatingsRDD)
#                              .map(lambda (id, (name, (nr, ar))) : (ar, name, nr, id)))
# movieLimitedAndSortedByRatingRDDwithMovieID = (movieNameWithIDAvgRatingsRDD
#                                                .filter(lambda (ar, name, nr, id) : nr > 500)
#                                                .sortBy(sortFunction, False))
# 
# for ratingsTuple in movieLimitedAndSortedByRatingRDDwithMovieID.take(50):
#     print ratingsTuple
...
myRatedMovieList = [(i[1]) for i in myRatedMovies]
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda rec : rec[1] not in myRatedMovieList)
                      .map(lambda rec: (myUserID, rec[0])))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda rec: (rec[0], rec[1][0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda rec: (rec.product, rec.rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .map(lambda rec: (rec[1][0], rec[0], rec[1][1])))
